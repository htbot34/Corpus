...
from ..budget import Budget, estimate_search_call
from ..cache import Cache
from .providers import SEARCH_MAX_TOKENS, SearchError, SearchProvider, SearchResult, SearchUsage
...
class SearchClient:
    ...
    def search(self, query: str, limit: int = 10) -> list[SearchResult]:
        ...
        with self.budget.reserved_for(estimate, f"search: {query[:60]}") as reservation:
            try:
            ...
            reservation.actual = self._charge(query, usage)

        self.searches_run += 1
        self.errors.extend(f"{query!r}: provider error {code}" for code in usage.errors)
    ...
    def _charge(self, query: str, usage: SearchUsage) -> float:
        ...
        cost = 0.0
        if usage.searches:
            cost += self.budget.charge_searches("web_search", usage.searches, note=note).cost
